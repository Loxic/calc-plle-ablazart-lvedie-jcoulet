...
subroutine Get_F(F,Mx,My,n_x1,n_xn,n_y1,n_yn,dx,dy,D,Dt,time,voisins,nb_probleme,rank,recouv,comm_cart)
...
! Nouvelle taille de boucle
    ...
    allocate(F(Mx*My),H1(Mx),B1(Mx),G1(My),D1(My),H2(Mx),B2(Mx),G2(My),D2(My))
...
!!! Condition de bord gauche : h(0,y) + Com' à droite
       ...
       call MPI_SEND(D1,My,MPI_REAL,voisins(3),101,comm_cart,statinfo)
...
!!! Condition de bord droit : h(1,y) + Com' à gauche
       ...
       do j=1,My
           G1(j) = F( Mx*(j-1) + 1 + recouv)
       end do
       call MPI_SEND(G1,My,MPI_REAL,voisins(1),101,comm_cart,statinfo)
       call MPI_RECV(G2,My,MPI_REAL,voisins(1),101,comm_cart,status,statinfo)
...
!!! Condition de bord bas : g(x,0) + Com' en haut
       ...
       do i=1,Mx
           H1(i) = F( Mx*(My-1-recouv) + i)
       end do
       call MPI_SEND(H1,Mx,MPI_REAL,voisins(2),101,comm_cart,statinfo)
       call MPI_RECV(H2,Mx,MPI_REAL,voisins(2),101,comm_cart,status,statinfo)
...
!!! Condition de bord haut : g(x,1) + Com' en bas
       ...
       call MPI_SEND(B1,Mx,MPI_REAL,voisins(4),101,comm_cart,statinfo)
       call MPI_RECV(B2,Mx,MPI_REAL,voisins(4),101,comm_cart,status,statinfo)
